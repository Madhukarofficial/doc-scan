...
app: Typer = Typer(
    help="Doc-scan is a chatbot based on OpenAI. It can also provide responses about queries on user-provided data.",
)


@app.command()
def migrate(
    chroma_path: str = os.path.expanduser("~/.local/share/doc-scan/chroma/"),
    data_path: str = os.path.expanduser("~/.local/share/doc-scan/data/"),
) -> None:
...
@app.command()
def version() -> None:
    """Print the version of the doc-scan package."""
    typer.echo(f"Doc-scan {__version__}")
