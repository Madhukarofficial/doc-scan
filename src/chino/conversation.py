...
class Conversation:
    def __init__(self) -> None:
        ...
        self.messages: List[Union[SystemMessage, HumanMessage]] = [
            SystemMessage(
                content="You are Doc-scan, a chatbot based on ChatGPT. 'Doc-scan' means 'document scanning and analysis'."
            ),
        ]
    ...
    def run_query(self, prompt: str) -> BaseMessage:
        with self.console.status("[i]thinking...[/i]"):
            query_text, query_sources = Query(
                prompt, os.path.expanduser("~/.local/share/doc-scan/chroma/")
            ).query_data()
